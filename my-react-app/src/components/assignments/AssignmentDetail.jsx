  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const assignmentData = await assignmentService.getAssignmentById(id, user.token);
...
const getCompleteImprovementPoints = (fullText) => {
  const section = getSectionContent(fullText, "3. Areas for Improvement", "4. Suggested Score (0-100)");
  ...
  try {
    ...
    const sections = [
      { 
        ...
        content: getSectionContent(content, "1. Summary of the Student's Approach", "2. Key Strengths"),
        icon: FaClipboardList
      },
      { 
        title: "Key Strengths", 
        content: getSectionContent(content, "2. Key Strengths", "3. Areas for Improvement"),
        icon: FaThumbsUp
      },
      { 
      ...
      { 
        ...
        content: getSectionContent(content, "4. Suggested Score (0-100)", "Resources for Improvement"),
        icon: FaStar
      },
      { 
        title: "Resources for Improvement", 
        content: getSectionContent(content, "Resources for Improvement", "Set of Questions to Help the Student Understand the Topics Better"),
        icon: FaBook
      },
      { 
        title: "Practice Questions", 
        content: getSectionContent(content, "Set of Questions to Help the Student Understand the Topics Better", "Learning Path (Chart Format)"),
        icon: FaQuestionCircle
      },
      { 
        title: "Learning Path", 
        content: getSectionContent(content, "Learning Path (Chart Format)", "Important Notes for the Student"),
        icon: FaProjectDiagram,
        isCode: true
      },
      { 
        title: "Important Notes", 
        content: getSectionContent(content, "Important Notes for the Student"),
        icon: FaLightbulb
      }
    ];
...
const getSectionContent = (fullText, sectionTitle, nextSectionTitle = null) => {
  ...
  let content = fullText.substring(startIndex + sectionTitle.length, endIndex).trim();
...
const renderSectionIfExists = (
  ...
  const colorClasses = {
    purple: { bg: 'bg-purple-50', text: 'text-purple-700', prose: 'prose-purple' },
    blue: { bg: 'bg-blue-50', text: 'text-blue-700', prose: 'prose-blue' },
    orange: { bg: 'bg-orange-50', text: 'text-orange-700', prose: 'prose-orange' },
    red: { bg: 'bg-red-50', text: 'text-red-700', prose: 'prose-red' },
    green: { bg: 'bg-green-50', text: 'text-green-700', prose: 'prose-green' },
    cyan: { bg: 'bg-cyan-50', text: 'text-cyan-700', prose: 'prose-cyan' },
    yellow: { bg: 'bg-yellow-50', text: 'text-yellow-800', prose: 'prose-yellow' }
  };
  
  return (
    <div className={`mb-8 ${halfWidth ? 'w-full' : ''}`}>
      <h5 className={`text-lg font-semibold ${colorClasses[color].text} mb-3 flex items-center`}>
        <span className="mr-2">{icon}</span>
        {displayTitle}
      </h5>
      <div className={`${colorClasses[color].prose} max-w-none ${colorClasses[color].bg} p-4 rounded-lg`}>
  ...
  return (
    <div className="p-8 relative min-h-screen">
      ...
      >
        ...
        >
          <h2 className="text-2xl font-bold text-purple-800">{assignment.title}</h2>
        ...
        <motion.div 
          ...
          className="bg-white border border-purple-100 rounded-xl p-6 shadow-md mb-6"
        >
          <h3 className="text-xl font-semibold text-purple-800 mb-4">Assignment Files</h3>
        ...
        {user.role === 'teacher' && (
          <motion.div 
            ...
            className="bg-white border border-purple-100 rounded-xl p-6 shadow-md mb-6"
          >
            <h3 className="text-xl font-semibold text-purple-800 mb-4">Manage Submissions</h3>
            <motion.div
              whileHover={{ scale: 1.05 }}
              whileTap={{ scale: 0.95 }}
            >
        ...
        {user.role === 'student' && (
          ...
          >
            <h3 className="text-xl font-semibold text-purple-800 mb-4">Your Submission</h3>
            
            {/* Submission Status Box */}
            <div className={`rounded-lg p-6 mb-6 ${submission ? 'bg-green-50 border-l-4 border-green-400' : 'bg-yellow-50 border-l-4 border-yellow-400'}`}>
              <div className="flex items-start">
                ...
                <div className="flex-1">
                  <h3 className={`text-lg font-medium ${submission ? 'text-green-800' : 'text-yellow-800'}`}>
                    {submission ? 'Submission Complete' : 'Submission Required'}
                  </h3>
                  <div className={`mt-2 text-sm ${submission ? 'text-green-700' : 'text-yellow-700'}`}>
                    <p>
                      {submission 
                  ...
                  <div className="mt-4">
                    ...
                    >
                      ...
                      {submission ? 'Update Submission' : 'Submit Your Solution Now'}
                    </motion.button>
                  </div>
                </div>
              </div>
    ...
    <div className="grid grid-cols-1 md:grid-cols-1 gap-6">
      <div>
        <p className="text-gray-600 flex items-center">
          <FaCalendarAlt className="text-purple-400 mr-2" />
          <span className="font-medium">Submitted on:</span> {formatDate(submission.submittedAt)}
        </p>
        
        {submission.grade && (
          <div className="mt-4 bg-purple-50 p-4 rounded-lg">
            <p className="text-lg font-semibold text-purple-800">
              Grade: <span className="text-2xl">{submission.grade}%</span>
            </p>
          </div>
        )}
        ...
        {submission.feedback && (
          <div className="mt-4">
            <h4 className="font-semibold text-purple-800">Teacher Feedback:</h4>
        ...
        <div className="mb-4 flex gap-4">
          ...
          {submission.summary && (
            <motion.button
              whileHover={{ scale: 1.05 }}
              whileTap={{ scale: 0.95 }}
              onClick={() => downloadSummaryAsPDF(submission.summary, `${assignment.title}_AI_Report`)}
            ...
            <div className="bg-white p-6 rounded-lg border border-gray-200 shadow-sm">
              ...
              <div className="mb-8">
                ...
                <div className="prose prose-purple max-w-none bg-purple-50 p-4 rounded-lg">
                  <ReactMarkdown>{getSectionContent(submission.summary, "1. Summary of the Student's Approach", "2. Key Strengths")}</ReactMarkdown>
                </div>
              </div>
              ...
              <div className="mb-8">
                ...
                <div className="prose prose-blue max-w-none bg-blue-50 p-4 rounded-lg">
                  <ReactMarkdown>{getSectionContent(submission.summary, "2. Key Strengths", "3. Areas for Improvement")}</ReactMarkdown>
                </div>
              </div>
              ...
              <div className="mb-8">
                ...
                <div className="prose prose-orange max-w-none bg-orange-50 p-4 rounded-lg">
                  <ReactMarkdown>{getCompleteImprovementPoints(submission.summary)}</ReactMarkdown>
                </div>
              </div>
              ...
              <div className="mb-8">
                ...
                <div className="prose prose-red max-w-none bg-red-50 p-4 rounded-lg">
                  <ReactMarkdown>{getSectionContent(submission.summary, "4. Suggested Score (0-100)", "Resources for Improvement")}</ReactMarkdown>
                </div>
              </div>
              ...
              <div className="mb-8 grid grid-cols-1 md:grid-cols-1 gap-6">
                <div>
                  ...
                  <div className="prose prose-green max-w-none bg-green-50 p-4 rounded-lg">
                    <ReactMarkdown>{getSectionContent(submission.summary, "Resources for Improvement", "Set of Questions to Help the Student Understand the Topics Better")}</ReactMarkdown>
                  </div>
                </div>
                
                <div>
                  <h5 className="text-lg font-semibold text-cyan-700 mb-3 flex items-center">
                    <FaQuestionCircle className="mr-2" />
                    Practice Questions
                  </h5>
                  <div className="prose prose-cyan max-w-none bg-cyan-50 p-4 rounded-lg">
                    <ReactMarkdown>{getSectionContent(submission.summary, "Set of Questions to Help the Student Understand the Topics Better", "Learning Path (Chart Format)")}</ReactMarkdown>
                  </div>
                </div>
              </div>
              
              <div className="mb-8">
                <h5 className="text-lg font-semibold text-indigo-700 mb-3 flex items-center">
                ...
                <div className="bg-indigo-50 p-4 rounded-lg overflow-x-auto">
                  <pre className="text-sm text-indigo-800 whitespace-pre-wrap font-mono">
                    {getSectionContent(submission.summary, "Learning Path (Chart Format)", "Important Notes for the Student")}
                  </pre>
                </div>
              </div>
              ...
              <div className="bg-yellow-50 border-l-4 border-yellow-400 p-4 rounded-r-lg">
                ...
                <div className="prose prose-yellow max-w-none">
                  <ReactMarkdown>{getSectionContent(submission.summary, "Important Notes for the Student")}</ReactMarkdown>
                </div>
              </div>
            </div>
          </div>
